...
public interface IReadonlyRepository<TEntity, TPrimaryKey, TAutocomplete, in TQueryParams, TAccessRightsPolicyParams>
    where TEntity : class, IEntity<TPrimaryKey> where TPrimaryKey : IEquatable<TPrimaryKey>
{
    ...
    Task<TEntity?> GetByIdAsync(TPrimaryKey id, RepositoryQueryOptions<TAccessRightsPolicyParams>? options = null,
    ...
    Task<IReadOnlyList<TEntity>> GetAllAsync(RepositoryQueryOptions<TAccessRightsPolicyParams>? options = null,
        CancellationToken token = default);
