...
{
    private const string DefaultDateSql = "now()";
    ...
    ///     <see cref="IAutoAuditCreatedAt" /> or <see cref="IAutoAuditLastModifiedAt" />.
    /// </summary>
    /// <param name="modelBuilder">The model builder.</param>
    public static void DefaultValueForAutoAuditDates(this ModelBuilder modelBuilder)
    {
        foreach (var entityType in modelBuilder.Model.GetEntityTypes())
        {
            ...
            if (typeof(IAutoAuditLastModifiedBy).IsAssignableFrom(entityType.ClrType))
                entityType.SetPropertyMaxLengthIfNotSet(nameof(IAutoAuditLastModifiedBy.LastModifiedBy), StringPropertyLengthConstants.Id);
        }
    }
